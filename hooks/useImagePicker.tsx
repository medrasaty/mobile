...
export interface UseImagePickerReturn {
  ...
  pickImage: (config?: ImagePickerConfig, source?: ImageSource) => Promise<boolean>;
  isLoading: boolean;
...
export default function useImagePicker(): UseImagePickerReturn {
  const [imageAsset, setImageAsset] = useState<ImagePickerAsset | undefined>(undefined);
  const [fileUpload, setFileUpload] = useState<fileUploadType | undefined>(undefined);
   ...
   */
  const processImageAsset = useCallback((asset: ImagePickerAsset): fileUploadType => {
  ...
  const pickImage = useCallback(async (
    ...
    try {
      const options: ImagePickerOptions = {
        ...
        allowsEditing: config?.allowsEditing !== undefined ? config.allowsEditing : true,
